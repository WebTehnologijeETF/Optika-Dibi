 ...
 <?php foreach ($rezultat as $Novosti) {?>	
	...
	<tr>
	 ...
	 <td><input type="text"   id ="tekstNovosti" name="tekstNovosti"  value="<?php echo $Novosti["Tekst"];?>" disabled=true >
	 	 ...
	 	 <td>	&nbsp;&nbsp;<input class="my-stylish-button" type="submit"name="obrisi" id ="ObrisiDugme" onclick="value='<?php echo $Novosti["IDNovosti"];?>'"></td>
	 </tr>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$rezultat = $veza->query("delete from Komentar where Novosti ='$brisanjeID'");
	$rez = $kom->query("delete from Novosti where IDNovosti ='$brisanjeID'");
     if (!$rez || !$rezultat) {
          $greska = $kom->errorInfo();
          print "SQL greška: " . $greska[2];
          exit();
